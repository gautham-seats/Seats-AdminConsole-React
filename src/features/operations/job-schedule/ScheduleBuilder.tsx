...
export function ScheduleBuilder({
  ...
  const select = (
    ...
    <FieldSelect
      id={`${id}-${field}`}
      label={label}
      value={String(parts[field])}
      disabled={disabled}
      onChange={next => change({ ...parts, [field]: Number(next) })}
      className={cn('shrink-0 tabular-nums', width)}
      options={options.map(([optionValue, text]) => ({ value: String(optionValue), label: text }))}
    />
  )
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* No overflow clip here: the chips lift on hover and their focus ring sits above the row. */}
      <div>
        {advanced ? (
