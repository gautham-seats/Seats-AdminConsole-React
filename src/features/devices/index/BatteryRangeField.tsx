...
// A battery shell the chosen band is clipped into, so the filter looks like what it filters.
const SHELL =
  'relative rounded-[3px] border-[1.5px] border-battery-shell bg-battery-track after:absolute after:top-1/2 after:-right-[3.5px] after:h-1.5 after:w-[2.5px] after:-translate-y-1/2 after:rounded-r-[2px] after:bg-battery-shell after:content-[""]'
...
export function BatteryRangeField({ id, range, onChange, labels }: BatteryRangeFieldProps) {
  ...
  // A reset or an outside change replaces the draft.
  if (seen !== range) {
    setSeen(range)
    setDraft(range)
  }
    ...
    <div role="group" aria-label={labels.title} className="flex min-w-0 flex-col gap-1.5">
      ...
      <div className="flex h-10 items-center gap-3 rounded-md border border-input bg-white px-3 shadow-sm">
        <span aria-hidden className={cn(SHELL, 'h-3.5 w-7 shrink-0')}>
          <span
            className={cn('absolute inset-0.5 rounded-[2px] transition-[clip-path] duration-300', BAND)}
